...
function openDatabase() {
  ...
  const db = SQLite.openDatabase("TPDB20.db");
...
function Totals() {
  ...
  return (
    <View style={styles.sectionContainer}>
      ...
      {totals.map(({ id, brand, numberOfRolls, sheetsPerRoll, price, sqFtPerRoll }) => (
        <Text style={styles.map} key={id}>
          {brand}:  {numberOfRolls}-pack  {("\n")}costs ${(price / numberOfRolls).toFixed(2)} per roll. {"\n"}
          That is ${((price / numberOfRolls) / sheetsPerRoll).toFixed(5)} per sheet{"\n"}or ${((price / numberOfRolls) / sqFtPerRoll).toFixed(5)} per square foot {"\n"}</Text>
